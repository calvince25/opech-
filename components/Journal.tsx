...
const Journal: React.FC<JournalProps> = ({ onArticleClick, fullPage = false }) => {
  ...
  return (
    <section id="journal" className={`${fullPage ? 'pt-0' : 'py-32'} bg-[#F5F2EB]`}>
      ...
      {fullPage && (
        <div className="relative h-[55vh] min-h-[420px] mb-20 overflow-hidden">
          <img
            ...
            loading="eager"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-stone-900/30 via-stone-900/20 to-[#F5F2EB] flex items-end pb-16 px-6 md:px-12">
            <motion.div 
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl font-serif text-white drop-shadow-lg">The Blog</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {posts.length === 0 ? (
              ...
              posts.map((article, index) => (
                ...
                >
                  <div className="w-full aspect-[4/3] overflow-hidden mb-8 bg-[#EBE7DE] rounded-xl">
                    <img 
                      src={article.image_url || '/blog-hero.jpg'}
                      alt={article.title} 
                  ...
                  <div className="flex flex-col flex-1 text-left">
                    ...
                    <h3 className="text-2xl font-serif text-[#2C2A26] mb-4 leading-tight group-hover:underline decoration-1 underline-offset-4">
                      {article.title}
                    </h3>
                    <p className="text-[#5D5A53] font-light leading-relaxed line-clamp-2 mb-6">{article.excerpt}</p>
                    <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#2C2A26] group-hover:gap-4 transition-all">
