...
export default function Admin() {
  ...
  return (
    <div className="pt-16 min-h-screen flex" style={{background:'#f0f0f1'}}>
      {/* WP Dark Sidebar */}
      ...
      <main className="flex-1 md:ml-52 p-6">
        {/* WP-style top page header */}
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <h1 className="text-lg font-bold text-stone-800 capitalize">
              {activeTab === 'dashboard' ? 'Dashboard' :
               activeTab === 'products' ? 'Products' :
               activeTab === 'blog' ? 'Posts' :
               activeTab === 'reviews' ? 'Reviews' :
               activeTab === 'messages' ? 'Messages' :
               activeTab === 'customers' ? 'Users' : 'Settings'}
            </h1>
            {activeTab === 'products' && !showProductModal && (
              <button onClick={() => handleOpenProductModal()} className="text-xs text-white bg-[#2271b1] hover:bg-[#135e96] px-3 py-1 rounded font-bold transition-colors">
                + Add New
              </button>
            )}
            {activeTab === 'blog' && !showCreatePost && (
              <button onClick={() => setShowCreatePost(true)} className="text-xs text-white bg-[#2271b1] hover:bg-[#135e96] px-3 py-1 rounded font-bold transition-colors">
                + Add New
              </button>
            )}
          </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-5">
            {/* WP-style compact stat boxes */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {stats.map((stat) => (
                <button 
                  key={stat.label} 
                  onClick={() => setActiveTab(stat.tab)}
                  className="w-full text-left bg-white border border-stone-200 p-4 flex items-center gap-3 shadow-sm hover:shadow-md hover:border-[#2271b1] hover:-translate-y-0.5 transition-all outline-none"
                >
                  <div className={`p-2 ${stat.bg} ${stat.color} rounded`}>
                    <stat.icon className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] text-stone-500 font-medium uppercase tracking-wider">{stat.label}</p>
